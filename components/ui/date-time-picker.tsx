...
import { add, format, isSameDay, startOfDay, endOfDay } from "date-fns";
...
// ======= utils ===========
/**
 * Format tanggal/waktu ke WIB (Asia/Jakarta).
 * Jika input string UTC, hasil akan dikonversi ke WIB.
 */
...
function Calendar({
  ...
  const boundaryStart = React.useMemo(() => {
    if (minDate) return new Date(minDate.getFullYear(), minDate.getMonth(), 1);
    const today = new Date();
    return new Date(today.getFullYear() - yearRange, 0, 1);
  }, [minDate, yearRange]);

  const boundaryEnd = React.useMemo(() => {
    if (maxDate) return new Date(maxDate.getFullYear(), maxDate.getMonth(), 1);
    const today = new Date();
    return new Date(today.getFullYear() + yearRange, 11, 1);
  }, [maxDate, yearRange]);

  const disableLeftNavigation = () => {
    if (props.month) {
      return (
        props.month.getMonth() === boundaryStart.getMonth() &&
        props.month.getFullYear() === boundaryStart.getFullYear()
      );
    }
    return false;
  };
  const disableRightNavigation = () => {
    if (props.month) {
      return (
        props.month.getMonth() === boundaryEnd.getMonth() &&
        props.month.getFullYear() === boundaryEnd.getFullYear()
      );
    }
    return false;
  };

  /** Disabled rule utk tanggal di luar range */
  const disabledDays = React.useMemo(() => {
    ...
    if (maxDate) {
      rules.push({ after: endOfDay(maxDate) });
    }
    if (props.disabled) {
      // gabungkan rules existing dari props
      if (Array.isArray(props.disabled)) rules.push(...props.disabled);
      else rules.push(props.disabled);
    }
    return rules;
  }, [minDate, maxDate, props.disabled]);
            ...
            <div className="inline-flex gap-2">
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  // cegah loncat keluar boundary
                  if (
                    newDate.getFullYear() < boundaryStart.getFullYear() ||
                    (newDate.getFullYear() === boundaryStart.getFullYear() &&
                      newDate.getMonth() < boundaryStart.getMonth())
                  ) {
                    newDate.setFullYear(boundaryStart.getFullYear());
                    newDate.setMonth(boundaryStart.getMonth());
                  }
                  if (
                    newDate.getFullYear() > boundaryEnd.getFullYear() ||
                    (newDate.getFullYear() === boundaryEnd.getFullYear() &&
                      newDate.getMonth() > boundaryEnd.getMonth())
                  ) {
                    newDate.setFullYear(boundaryEnd.getFullYear());
                    newDate.setMonth(boundaryEnd.getMonth());
                  }
...
type DateTimePickerProps = {
  ...
  /** NEW: batas minimum & maksimum yang diperbolehkan */
  minDate?: Date;
  maxDate?: Date;
} & Pick<
  DayPickerProps,
...
>(
  (
    ...
    // helper untuk set & clamp waktu
    const applyChange = (d?: Date) => {
      if (!d) {
        onChange?.(undefined);
    ...
    const handleMonthChange = (newDay: Date | undefined) => {
      ...
      // jaga "preview month" tanpa menabrak batas
      if (!defaultPopupValue) {
        ...
        onMonthChange?.(clampDate(newDay, minDate, maxDate));
        setMonth(clampDate(newDay, minDate, maxDate));
        return;
      }
      ...
      const clamped = clampDate(newDateFull, minDate, maxDate);
    ...
    const onSelect = (newDay?: Date) => {
      ...
      // ketika pilih tanggal, bawa jam-menit-detik dari "month"
      newDay.setHours(
        month?.getHours() ?? 0,
        month?.getMinutes() ?? 0,
        month?.getSeconds() ?? 0
      ...
      // Kalau sama dgn minDate/maxDate (di level hari), clamp waktunya
      if (minDate && isSameDay(newDay, minDate)) {
        // jika waktu sebelum minDate, paksa ke minDate time
        if (newDay < minDate) {
          newDay.setHours(
            minDate.getHours(),
            minDate.getMinutes(),
      ...
      if (maxDate && isSameDay(newDay, maxDate)) {
        // jika waktu setelah maxDate, paksa ke maxDate time
        if (newDay > maxDate) {
          newDay.setHours(
            maxDate.getHours(),
            maxDate.getMinutes(),
      ...
      <Popover>
        ...
        <PopoverContent className="w-auto p-0">
          <Calendar
            ...
            onSelect={(newDate) => {
              if (newDate) {
                // jaga jam-menit-detik dari state "month"
                newDate.setHours(
                  month?.getHours() ?? 0,
                  month?.getMinutes() ?? 0,
                  month?.getSeconds() ?? 0
            ...
            minDate={minDate}
            maxDate={maxDate}
            // juga batasi navigasi "fromMonth"/"toMonth" untuk keamanan
            fromMonth={
              minDate ?? new Date(new Date().getFullYear() - yearRange, 0, 1)
            }
            toMonth={
              maxDate ?? new Date(new Date().getFullYear() + yearRange, 11, 1)
            }
            {...props}
          />
          {granularity !== "day" && (
            <div className="border-border border-t p-3">
              <TimePicker
                onChange={(value) => {
                  // clamp di sini agar perubahan jam/menit/detik tidak keluar range
                  if (!value) {
                    applyChange(undefined);
                    return;
                  }
                  // Jika tanggal sama dgn minDate/maxDate, pastikan tidak menyalahi batas waktu
                  let next = new Date(value);

                  if (minDate && isSameDay(next, minDate) && next < minDate) {
                    next = new Date(minDate);
                  }
                  if (maxDate && isSameDay(next, maxDate) && next > maxDate) {
                    next = new Date(maxDate);
                  }

                  // Jika beda hari, tetap clamp global
              ...
              {/* Info batas (opsional) */}
              {(minDate || maxDate) && (
                <div className="px-1 pt-2 pb-1 text-xs text-muted-foreground">
                  {minDate && <div>Min: {format(minDate, "PPpp")}</div>}
                  {maxDate && <div>Max: {format(maxDate, "PPpp")}</div>}
                </div>
              )}
            </div>
          )}
